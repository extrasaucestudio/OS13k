...
programs = 
{
'Help':'<h2><i>Glory to RemixWebOS!</i></h2><p>The RemixWebOS is tiny control system made by workers of great nation for JS13k.<p>All citizens are asked to submit little programs for inclusion.<p>😎🤚🚩 Ministry of RemixWebOS<p>Project Links...<ul><li><a href=https://github.com/KilledByAPixel/RemixWebOS target=_blank>GitHub</a><li><a href=https://discord.gg/n8vxUcZ target=_blank>Discord</a></ul>',
'Marathon':`// Click = Jump
// R = Restart
...
Q+=V?P/29:P/6
Y-=V-=.06
Y>H?Y+(V=0)>H+3?(P&&RemixWebOS.PlaySeed(100,2),P=0,RemixWebOS.Trophy('🏃','Marathon','High Score',j)):(Y=H,V+=I.mousepress[0]?(RemixWebOS.PlaySeed(19,.1),2*P):0):0
...
j>99&&RemixWebOS.Trophy('🎖️','Marathon','100!')
if((F/47|0)-(T/47|0))RemixWebOS.PlaySeed(3,0)
`,
'Zeremin':`// Mouse = Play
// Wheel = Change Sound
c.style.filter='invert('
...
x.fillText(S,X,Y),
RemixWebOS.PlaySeed(S,X/2e3,.4,0,RemixWebOS.Note(9.99-Y/90|0),1)
`,
...
'Medal Cabinet':'<style>body{background:#223}</style><body><script>RemixWebOSReload=()=>{for(t of(this.document.body.innerHTML="",parent.RemixWebOS.Trophies()))this.document.body.innerHTML+=`<div style="display:inline-block;font-family:arial;padding:9;overflow:hidden;text-align:center;background:linear-gradient(#eee,#888);width:120;height:120;margin:9;font-size:18;box-shadow:5px 5px 9px;border:2px solid;border-radius:99px"title="${i=t[0]||`🎖️`} ${n=t[2]}\n${g=t[1]}\n${m=t[3]}" \nonclick=parent.RemixWebOS.Speak("${n}")>\n<div style="padding:3;white-space:nowrap;font-size:28;text-shadow:2px 2px 4px">${i}</div><div><b>${n}</b><br><i>${g}</i><br><div style=font-size:14>`+m},RemixWebOSReload()</script>',
'Notebook':'<body style=font-size:23 id=b contenteditable=true spellcheck=false><script>b.focus();b.innerHTML=localStorage.RemixWebOSN||"";b.oninput=e=>localStorage.RemixWebOSN=b.innerHTML;RemixWebOSReload=e=>b.innerHTML=localStorage.RemixWebOSN="";</script>',
...
'Visualizer':'t?x.fillRect(0,0,c.width|=0,2e3):h=[];for(i=32;i--;x.clearRect(60*i,1080-(h[i]=h[i]>m?h[i]-1:m),55,-9))m=5*RemixWebOS.GetAnalyserData(i)*299,x.fillStyle=R(m/2),x.fillRect(60*i,1080,55,-m)',
};
...
// RemixWebOS Seralization
    ...
    Save()
    {
        ...
        finishedStartup && (localStorage.RemixWebOS2 = JSON.stringify
        ([
            trophies,
            settings,
            programInfos,
...
// load save data
if (localStorage.RemixWebOS2)
    [trophies, settings, programInfos, startProgramId] = JSON.parse(localStorage.RemixWebOS2);
...
// welcome message
RemixWebOS.Trophy('🚩','','Glory to RemixWebOS!');
